...
def create_triangle_plane_mesh(plane_model):
    a, b, c, d = plane_model

    # Define the vertices for the plane triangles
    vertices = np.array([
        [5, -5, (-a * 5 - b * -5 - d) / c],
        [5, 5, (-a * 5 - b * 5 - d) / c],
        [-5, -5, (-a * -5 - b * -5 - d) / c],
    ...
    # Define the triangles (faces) using vertex indices
    triangles = np.array([
        [0, 1, 2],
        [1, 3, 2]
    ])
...
class O3DPointCloud():
    ...
    def create_from_pcd(self, pcd):
        self.pcd = pcd
        self.max_points = len(pcd.points)
...
def pcd_normal_vectors(pcd, voxel_size=0.01, radius=0.1, max_nn=30):
...
def get_kdtree_flann(pcd):
    return o3d.geometry.KDTreeFlann(pcd)

def find_closest_point(pcd, kdtree, point, knn=1, only_index=False):
    ...
    if only_index:
        return idx[0]
    return pcd.normals[idx[0]], idx[0]
